...
class AddressValidator:
    ...
    def __init__(self, provider="smartystreets", api_key=None, auth_id=None, auth_token=None, user_id=None):
        """
        Initialize address validator.

        Args:
            provider: "smartystreets" or "google" (default: smartystreets)
        ...
        # Address validation cache (reduces duplicate API calls in batch processing)
        self.address_cache = {}

        if self.provider == "smartystreets":
            ...
            raise ValueError(f"Unsupported provider: {provider}. Use 'smartystreets' or 'google'. USPS has been deprecated due to rate limiting issues.")
    ...
    def _validate_smartystreets(self, street, city, state, zipcode) -> Dict:
        """
        Validate using SmartyStreets US Street API (CASS certified, production-grade).

        Returns all fields needed for IMB generation:
        - DPV status (delivery point validation)
        - ZIP+4
        - Delivery Point
        - Carrier Route
        - LACSLink (address conversion tracking)
        - Vacancy indicator
        """
        params = {
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(f"SmartyStreets API error: {response.status_code} - {response.text}")
                raise Exception(f"SmartyStreets API error: {response.status_code} - {response.text}")

            data = response.json()

            # Handle no results (address not found or invalid)
            if not data or len(data) == 0:
                return {
                    ...
                    'lacslink_indicator': '',
                    'vacant': '',
                    'message': 'Address not found or does not match USPS records'
                }

            # Extract first result (SmartyStreets returns array)
            result = data[0]
            ...
            delivery_point = components.get('delivery_point', '')
            carrier_route = metadata.get('carrier_route', '')

            # DPV (Delivery Point Validation) status
            dpv_match_code = metadata.get('dpv_match_code', 'N')
            dpv_status = 'Valid' if dpv_match_code == 'Y' else 'Invalid' if dpv_match_code == 'N' else 'Uncertain'

            # LACSLink (address conversion tracking)
            lacslink_code = metadata.get('lacslink_code', '')
            lacslink_indicator = metadata.get('lacslink_indicator', '')

            # Vacancy indicator
            vacant = metadata.get('vacant', '')

            # Check for non-DPV addresses (warn but still process)
            if dpv_match_code != 'Y':
                logger.warning(f"Non-DPV address: {street}, {city}, {state} (DPV: {dpv_match_code})")

            # Check for vacant addresses
            if vacant == 'Y':
                logger.warning(f"Vacant address detected: {street}, {city}, {state}")

            # Check for LACSLink conversions
            if lacslink_code or lacslink_indicator:
                logger.info(f"LACSLink conversion: {street} (code: {lacslink_code}, indicator: {lacslink_indicator})")

            # Build routing code: ZIP5 + ZIP4 + Delivery Point (11 digits)
            ...
            message_parts = ['Address validated successfully via SmartyStreets']
            if vacant == 'Y':
                message_parts.append('VACANT')
            if lacslink_code:
                message_parts.append(f'LACSLink: {lacslink_code}')
            if dpv_match_code != 'Y':
                message_parts.append(f'Non-DPV ({dpv_match_code})')
            ...
            logger.error(f"SmartyStreets API request failed: {str(e)}")
            raise Exception(f"SmartyStreets API request failed: {str(e)}")
